...
import { requestPasswordResetValidationRules, resetPasswordValidationRules, validateRequest } from "../../utils/validations_util";
...
export const requestPasswordReset = async (req: Request, res: Response) => {
  ...
  try {
    // Validation
    const validationErrors = await validateRequest(req, res, requestPasswordResetValidationRules);
    ...
    return sendErrorResponse({
      ...
      errorDetails: "An unexpected error occurred during password reset request",
      status: 500,
    });
  }
};
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    // Validation
    const validationErrors = await validateRequest(req, res, resetPasswordValidationRules);
    if (validationErrors !== "validation successful") {
      return sendErrorResponse({
        res,
        message: "Invalid input",
    ...
    if (recoveryMessage === "deleted") {
      return sendErrorResponse({
        ...
        errorDetails: "The recovery period has ended. Your account is scheduled for permanent deletion.",
        status: 403,
      });
    }
    if (recoveryMessage) {
    ...
    return sendErrorResponse({
      ...
      message: "Server error",
      errorCode: "SERVER_ERROR",
      errorDetails: "An unexpected error occurred during password reset, Please try again later.",
      status: 500,
    });
  }
};
